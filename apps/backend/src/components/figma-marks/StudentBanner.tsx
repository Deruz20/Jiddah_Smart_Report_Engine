...
import type { EnrollmentData } from './SelectionBar'
  ...
  return (
    <div style={{
      ...
      <div style={{ flexShrink: 0 }}>
        ...
        <div style={{ color: '#9ca3af', fontSize: 12, marginTop: 2 }}>{student.admission_number}</div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        <Chip label="Class" value={student.circular_class} color="green" />
        {student.section && <Chip label="Section" value={student.section} color="orange" />}
        {/* Arabic level */}
      ...
      <div style={{ marginLeft: 'auto', flexShrink: 0 }}>
        <div style={{
          ...
          <span style={{ color: '#c2410c', fontSize: 12, fontWeight: 700, textTransform: 'uppercase' }}>{examType}</span>
...
function Chip({ label, value, color }: { label: string; value: string; color: 'green' | 'orange' }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ color: s.labelColor, fontSize: 11, fontWeight: 600, letterSpacing: '0.04em', textTransform: 'uppercase' }}>
        {label}
      </span>
      <span style={{ color: s.valueColor, fontSize: 13, fontWeight: 700 }}>
        {value}
      </span>
    </div>
  )
