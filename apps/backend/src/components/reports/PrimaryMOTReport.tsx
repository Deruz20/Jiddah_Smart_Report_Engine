...
export default function PrimaryMOTReport({ reportData }: any) {
  ...
  return (
    ...
    .primary-mot-report .line { border-bottom: 2px dotted #555; width: 150px; height: 18px; }
    ...
    .primary-mot-report .grade-table th { background: #d6b14c; color: #000; padding: 4px; border: 1px solid #333; font-size: 10px; }
    .primary-mot-report .grade-table td { border: 1px solid #333; text-align: center; padding: 4px; font-size: 11px; }
      ...
      <div className="primary-mot-report font-poppins">
        ...
        <div className="info">
          <div className="info-row">
            <div className="field">Class: <div className="line" style={{ paddingLeft: '8px', paddingTop: '2px' }}>{reportData?.student?.class_name}</div></div>
            <div className="field">Term: <div className="line" style={{ paddingLeft: '8px', paddingTop: '2px' }}>{reportData?.term?.label}</div></div>
            <div className="field">Year: <div className="line" style={{ width: '100px', paddingLeft: '8px', paddingTop: '2px' }}>{reportData?.term?.academic_year}</div></div>
          </div>
          <div className="info-row" style={{ marginBottom: 0 }}>
            <div className="field" style={{ width: '100%' }}>
              Pupil’s Name: <div className="line" style={{ width: '100%', paddingLeft: '8px', paddingTop: '2px' }}>{reportData?.student?.name}</div>
            </div>
          </div>
          <div className="info-row" style={{ marginTop: '12px' }}>
            <div className="field">Position: <div className="line" style={{ paddingLeft: '8px', paddingTop: '2px' }}>{reportData?.circular?.position ?? '--'}</div></div>
            <div className="field">Out Of: <div className="line" style={{ paddingLeft: '8px', paddingTop: '2px' }}>{reportData?.circular?.total_students ?? '--'}</div></div>
            <div className="field">Division: <div className="line" style={{ paddingLeft: '8px', paddingTop: '2px' }}>{reportData?.circular?.division ?? '--'}</div></div>
          </div>
        </div>
        ...
        <table className="main-table">
          <tbody>
            <tr className="subhead">
              ...
              <th>MIDTERM MARK</th>
              <th>MIDTERM AGG</th>
              <th style={{ width: '25%' }}>SUBJECT TEACHER’S COMMENT</th>
            </tr>
            {reportData?.circular?.subjects?.map((subject: any) => (
              <tr key={subject.subject_name}>
                ...
                <td style={{ fontStyle: 'italic', color: '#444' }}>{subject.remark ?? ''}</td>
              </tr>
            ))}
            <tr>
              <td><b>TOTAL</b></td>
              <td>{reportData?.circular?.total}</td>
              <td>{reportData?.circular?.mot_aggregate ?? '--'}</td><td></td>
            </tr>
          </tbody>
        </table>
