...
import { schoolSettingsSchema, type SchoolSettingsForm } from "@/lib/validation";
...
export default function SettingsPage() {
  const { terms, loading: termsLoading, error: termsError, refetch: refetchTerms } = useTerms();
  ...
  const schoolSettingsForm = useForm<SchoolSettingsForm>({
    resolver: zodResolver(schoolSettingsSchema),
    mode: "onBlur",
    defaultValues: defaultSchoolValues,
  });

  const { register, handleSubmit, setValue, reset, formState: { errors, isValid } } = schoolSettingsForm;
  ...
  const fetchSchoolSettings = async () => {
    ...
    try {
      const response = await api.get<{ data: Partial<SchoolSettingsForm> }>(ENDPOINTS.settings.school);
      const payload: any = response.data ?? {};
      const loadedValues: SchoolSettingsForm = {
        school_name: payload.school_name ?? payload.name ?? defaultSchoolValues.school_name,
        address: payload.address ?? defaultSchoolValues.address,
        district: payload.district ?? defaultSchoolValues.district,
        phone_1: payload.phone_1 ?? payload.phone ?? defaultSchoolValues.phone_1,
      ...
      setNextTermStart(payload.next_term_start ?? "");
    } catch (err) {
      logger.error("Failed to load school settings", err);
      setSettingsError("Could not load settings. Is the backend running?");
    } finally {
  ...
  const handleSave = handleSubmit(async (values) => {
    ...
    try {
      const diff = Object.entries(values).reduce((acc, [key, value]) => {
        const previousValue = initialSchoolSettings?.[key as keyof SchoolSettingsForm];
      ...
      logger.debug("School settings save diff", {
        changedFields: Object.keys(diff),
        diff,
      });

      await api.patch(ENDPOINTS.settings.school, {
      ...
      logger.error("School settings save failed", err);
          ...
          <div className="space-y-5">
            ...
            <div className="p-5 rounded-2xl" style={{ background: "#F9FAFB", border: "1px solid #E5E7EB" }}>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <AnimatedButton className="flex items-center gap-2 px-4 py-2 rounded-xl mb-2 hover:opacity-90"
                    style={{ background: "#10B981", color: "white", fontSize: "13px" }}
                    onClick={() => toast.info("Logo upload opened")}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={key} className={key === "school_name" || key === "motto" ? "col-span-2" : ""}>
                  ...
                  <input
                    {...register(key as keyof SchoolSettingsForm)}
                    defaultValue={defaultSchoolValues[key as keyof SchoolSettingsForm] as string}
              ...
              <div className="col-span-2">
                ...
                <textarea
                  {...register("address")}
                  defaultValue={defaultSchoolValues.address}
          ...
          <div className="space-y-5">
            ...
            {[
              ...
              <div key={label} className="flex items-center justify-between p-4 rounded-2xl" style={{ border: "1px solid rgba(0,0,0,0.07)" }}>
                ...
                <AnimatedButton className="relative w-11 h-6 rounded-full transition-all duration-200" style={{ background: enabled ? "#10B981" : "#E5E7EB" }}>
                  <span className="absolute top-1 left-1 w-4 h-4 rounded-full bg-white transition-all duration-200" style={{ transform: enabled ? "translateX(20px)" : "translateX(0)" }} />
                </AnimatedButton>
              </div>
            ))}
          </div>
  ...
  return (
    <div>
      <HeroSection
        title="Settings"
        subtitle="Configure platform preferences and system settings for your school"
      />
      ...
      <div className="flex gap-6">
          ...
          <div className="w-56 flex-shrink-0">
            <div className="rounded-2xl overflow-hidden" style={{ background: "white", border: "1px solid rgba(0,0,0,0.07)", boxShadow: "0 1px 8px rgba(0,0,0,0.04)" }}>
              {settingsSections.map(({ id, label, icon: Icon }) => (
                ...
                className="w-full flex items-center gap-3 px-4 py-3.5 text-left border-b transition-all hover:bg-gray-50"
                style={{
        ...
        <ScrollReveal delay={0.1}>
          <div className="flex-1 rounded-2xl p-6" style={{ background: "white", border: "1px solid rgba(0,0,0,0.07)", boxShadow: "0 1px 8px rgba(0,0,0,0.04)" }}>
            {renderSection()}
            ...
            <div className="flex items-center justify-between mt-8 pt-6 border-t" style={{ borderColor: "rgba(0,0,0,0.07)" }}>
              <AnimatedButton
                ...
                className="px-5 py-2.5 rounded-xl hover:bg-gray-50 transition-all"
              ...
              <AnimatedButton
                onClick={handleSave}
                disabled={saving}
                className="flex items-center gap-2 px-6 py-2.5 rounded-xl hover:opacity-90 disabled:opacity-60 transition-all"
