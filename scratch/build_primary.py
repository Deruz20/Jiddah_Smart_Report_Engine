      ...
      <div className="report-page">
        ...
        <header className="header">
            <div className="school-left">
                <h1>JIDDAH ISLAMIC NURSERY<br/><span className="red-text">AND PRIMARY SCHOOL - Nsaggu</span></h1>
                <p>P.O.Box 34008,Kampala(U) Email:jiddahislamicnurseryandpri@gmail.com</p>
                <p>Telephone: +256 (0)744950042 / +256 (0)705316961</p>
            </div>

            <div className="header-center">
                <div className="bismillah">بِسْمِ اللّٰهِ الرَّحْمٰنِ الرَّحِيْمِ</div>
                <div className="logo"><img src="/school_budge.jpeg" alt="Logo" /></div>
                <div className="report-badge">UPPER REPORT FORM</div>
            </div>

            <div className="header-right">
                <h2>مدرسة جدة الإسلامية للروضة<br/>والابتدائية بنساغو</h2>
            </div>
        </header>
        ...
        <section className="pupil-info">
            <div className="info-col-left">
                <div className="info-row">
                    ...
                    <div className="line" style={{ flex: 1 }}>{reportData?.student?.name}</div>
                </div>
                <div className="info-row">
                    <span className="label">Year:</span><div className="line">{reportData?.term?.academic_year ?? '--'}</div>
                ...
                <div className="info-row">
                    ...
                    <span className="label">Division:</span><div className="line">{reportData?.circular?.division ?? '--'}</div>
                </div>
            </div>
            ...
            <div className="info-col-right" style={{ paddingRight: '20px' }}>
                <div className="info-row">
                    ...
                    <div className="line" style={{ borderBottomColor: '#000', borderBottomStyle: 'dotted' }}>{reportData?.student?.arabic_name || transliterateEnglishToArabic(reportData?.student?.name || '')}</div>
                </div>
                <div className="info-row">
                    <span className="label">الفصل:</span><div className="line" style={{ borderBottomColor: '#000', borderBottomStyle: 'dotted' }}>{reportData?.student?.theology_class_arabic ?? reportData?.student?.class_name}</div>
                    <span className="label">الفترة:</span><div className="line" style={{ borderBottomColor: '#000', borderBottomStyle: 'dotted' }}>{toAr(reportData?.term?.term_number)}</div>
                    <span className="label">عام:</span>
                    <div className="line" style={{ flex: 0.5, borderBottomColor: '#000', borderBottomStyle: 'dotted' }}>{toAr(toHijri(reportData?.term?.academic_year || 2024))}</div><span className="label">هـ</span>
                    <div className="line" style={{ flex: 0.5, borderBottomColor: '#000', borderBottomStyle: 'dotted' }}>{toAr(reportData?.term?.academic_year)}</div><span className="label">م</span>
                </div>
                <div className="info-row">
                    <span className="label">الترتيب:</span><div className="line" style={{ borderBottomColor: '#000', borderBottomStyle: 'dotted' }}>{toAr(reportData?.theology?.position)}</div>
                    <span className="label">عددالطلبة:</span><div className="line" style={{ borderBottomColor: '#000', borderBottomStyle: 'dotted' }}>{toAr(reportData?.theology?.total_students)}</div>
                </div>
            </div>
            )}
        </section>
        ...
        <main className="tables-section">
            <div className="academic-table" style={!showTheologyPanel ? { width: '100%' } : {}}>
                <table>
                    <thead>
                        <tr>
                            <th colSpan={2} rowSpan={2} style={{ width: '22%' }}>SUBJECTS</th>
                            <th colSpan={5} className="table-title">COMPARATIVE PERFORMANCE</th>
                        </tr>
                        <tr>
                            <th colSpan={2}>BEGINNING OF<br/>TERM</th>
                            <th colSpan={2}>MIDTERM</th>
                            <th colSpan={2}>END OF TERM</th>
                            <th rowSpan={2} style={{ width: '25%' }}>SUBJECT<br/>TEACHER'S<br/>COMMENT</th>
                        </tr>
                        <tr>
                            <th colSpan={2}></th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colSpan={2} style={{ textAlign: 'left', paddingLeft: '4px', fontWeight: 900 }}>TOTAL</td>
                ...
                <div style={{ fontWeight: 800, fontSize: '12px', marginTop: '4px' }}>Grading</div>
                <table className="grading-key">
                    <tbody>
                        <tr className="grade-row"><th>Grade</th><td>D1</td><td>D2</td><td>C3</td><td>C4</td><td>C5</td><td>C6</td><td>P7</td><td>P8</td><td>F9</td></tr>
                        <tr><th>Marks</th><td>85-100</td><td>75-84</td><td>70-74</td><td>60-69</td><td>55-59</td><td>50-54</td><td>40-49</td><td>35-39</td><td>0-34</td></tr>
                    </tbody>
                </table>
            </div>
            ...
            <div className="theology-table">
                <table>
                    <thead>
                        <tr>
                            <th rowSpan={3} style={{ width: '25%' }}>المواد</th>
                            <th colSpan={4} className="table-title">نتائج المواد الشرعية</th>
                            <th rowSpan={3} style={{ width: '25%' }}>الملاحظات</th>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <th>الدرجة الكبرى</th><th>الدرجة الصغرى</th>
                            <th>الدرجة الكبرى</th><th>الدرجة الصغرى</th>
                        </tr>
                    </thead>
                    <tbody>
                        {arabicSubjects.map((arabicName) => {
                          ...
                          return (
                            <tr key={arabicName}>
                                <td style={{ textAlign: 'right', paddingRight: '4px' }}>{arabicName}</td>
                                <td>{toAr(100)}</td>
                                <td>{toAr(subject?.mot_score)}</td>
                                <td>{toAr(100)}</td>
                                <td>{toAr(subject?.eot_score)}</td>
                                <td style={{ textAlign: 'right', paddingRight: '4px' }}>{subject?.theology_remark ?? ''}</td>
                            </tr>
                          );
                        })}
                        <tr>
                            <td style={{ textAlign: 'right', paddingRight: '4px', fontWeight: 900 }}>المجموع</td>
                ...
                <div className="theology-comment">
                    ...
                    <div className="line">{getTheologyComment(reportData?.theology?.total ?? null)}</div>
                </div>
            </div>
            )}
        </main>
        ...
        <footer className="footer">
            ...
            <div className="dates-row">
                <div><span className="label">This term ends On: </span><span className="date">{reportData?.term?.end_date || '________________'}</span></div>
                <div><span className="label">Next term Begins On: </span><span className="date">{reportData?.term?.next_term_start_date || '________________'}</span></div>
            </div>
...
mot_content = TEMPLATE.replace('PrimaryEOTReport', 'PrimaryMOTReport')
mot_content = mot_content.replace('UPPER REPORT FORM', 'LOWER REPORT FORM')

with open('src/components/reports/PrimaryMOTReport.tsx', 'w', encoding='utf-8') as f:
    f.write(mot_content)

bot_content = TEMPLATE.replace('PrimaryEOTReport', 'PrimaryBOTReport')
bot_content = bot_content.replace('UPPER REPORT FORM', 'LOWER REPORT FORM')
