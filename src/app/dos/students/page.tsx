...
export default async function StudentsManagementPage() {
  ...
  const isTheology = profile?.role?.toLowerCase().includes('theology') || profile?.subject?.toLowerCase().includes('theology')
  ...
  if (isTheology) {
    ...
    // Secular DOS might oversee all students or just those in secular classes.
    // Usually, Secular DOS oversees standard circular classes.
